document.addEventListener('DOMContentLoaded', () => {
    ...
    speakButton.addEventListener('click', () => {
        ...
        if (isSpeaking) {
            ...
            utterance.lang = translations[currentLang].lang; // 現在の言語を設定

            utterance.onstart = () => {
                isSpeaking = true;
                speakButton.style.opacity = '0.5'; // 再生中は半透明に
            };

            utterance.onend = () => {
                isSpeaking = false;
                speakButton.style.opacity = '1'; // 終了したら元に戻す
            };

            utterance.onerror = (event) => {
                console.error('SpeechSynthesisUtterance.onerror', event);
                isSpeaking = false;
                speakButton.style.opacity = '1'; // エラー時も元に戻す
            };
